...
def main():
    ...
    if os.path.exists(gcsc_ckpt):
        ...
        print("  MLS:")
        print(f"    AUROC (Near/Far): {compute_auroc(test_scores, near_scores):.4f} / {compute_auroc(test_scores, far_scores):.4f}")
        print(f"    Rejection @ FPR95 (Test/Near/Far): {1.0 - compute_rejection_rates(test_scores, tau):.4f} / {compute_rejection_rates(near_scores, tau):.4f} / {compute_rejection_rates(far_scores, tau):.4f}")
